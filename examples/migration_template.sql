-- A template to provide migration instructions for SQLite.

-- Simple changes like "rename table", "rename column", "add column", "drop column" are
-- supported by SQLite. They can be done in a simple transaction like this:
...
-- For more other changes it is critically important to follow the 12-step procedure
-- described in https://www.sqlite.org/lang_altertable.html to avoid data loss:
...
-- 7. Change the name of new_X to X using:
...
--  triggers, indexes, and views saved from step 3 above as a guide, making changes as
--  appropriate for the alteration.

-- 9. If any views refer to table X in a way that is affected by the schema change,
--  then drop those views using DROP VIEW and recreate them with whatever changes are
...
--  PRAGMA foreign_key_check to verify that the schema change did not break any foreign
--  key constraints.
